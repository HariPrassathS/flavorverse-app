...
function checkAdminAccess() {
    ...
    if (!currentUser.role || (!currentUser.role.includes('ADMIN') && !currentUser.role.includes('SUPER_ADMIN'))) {
...
async function loadAdminProfile() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            showMessage('Error loading admin profile information', 'error');
        }
    } catch (error) {
...
function displayProfile(profile) {
    document.getElementById('display-fullName').textContent = profile.fullName || 'Administrator';
    ...
    // Update admin level in system info
    document.getElementById('admin-level').textContent = formatRole(profile.role);
...
// Close system information modal
...
async function updateProfile(event) {
    ...
    if (updateData.newPassword) {
        ...
        if (updateData.newPassword.length < 8) {
            showMessage('New password must be at least 8 characters long for security', 'error');
            return;
        }
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            showMessage(errorText || 'Error updating admin profile', 'error');
        }
    } catch (error) {
